<?php
    ...
    function buscarRepetido(string $tipoDisco, object $conexion): int {
        $consulta = "CALL sp_existeTipoDisco(?)";         
        $resultado = $conexion->prepare($consulta);
        $resultado->bindValue(1, $tipoDisco);
        $resultado->execute(); 
    ...
    $verificar = buscarRepetido($tipoDisco,  $conexion);
    $data = null;
    ...
    switch($opcion){
        case 1:
            if($verificar == 1){
                ...
                $consulta = "CALL sp_insertarTipoDisco(?)";         
                $resultado = $conexion->prepare($consulta);
                $resultado->bindValue(1, $tipoDisco);
                $resultado->execute();
                $data = 0; 
        ...
        case 2: 
            if($verificar == 1){
                ...
                $consulta = "CALL sp_actualizarTipoDisco(?, ?)";   
                $resultado = $conexion->prepare($consulta);
                $resultado->bindValue(1, $id);
                $resultado->bindValue(2, $tipoDisco);
                $resultado->execute();
            ...
            break;       
        case 3:        
            $consulta = "CALL sp_eliminarTipoDisco(?) ";
            $resultado = $conexion->prepare($consulta);
